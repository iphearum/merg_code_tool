...
//               </div>';
    ...
    $return_string .= '<div id="'.$anime_id.'" class="aaa article_container wpestate_latest_listings_sh bottom-'.$type.' '.$class.' "  '
            . 'data-type="'.$type.'" '
...
function wpestate_slider_recent_posts_pictures($attributes)
{
    ...
    while ($recent_posts->have_posts()): $recent_posts->the_post();
        ...
        if ($featured == 1) {
            ...
            print '<div id="row2_5" class=" slider_prop_wrapper  '.$three_per_row_class.'">';
...
function wpestate_advanced_search_function($attributes, $content = null)
{
    ...
    if ($custom_advanced_search=='yes') {
        ...
        if ($adv_search_type==6) {
            ...
            } elseif ($adv6_taxonomy=='property_action_category') {
                $search_field="types";
            } elseif ($adv6_taxonomy=='property_city') {
                $search_field="cities";
            } elseif ($adv6_taxonomy=='property_area') {
                $search_field="areas";
            } elseif ($adv6_taxonomy=='property_county_state') {
        ...
        foreach ($adv_search_what as $key=>$search_field) {
            ...
            wpestate_show_search_field('shortcode', $search_field, $action_select_list, $categ_select_list, $select_city_list, $select_area_list, $key, $select_county_state_list, $select_sub_categories_list);
            print '</div>';
        } // end foreach
        $templates = ob_get_contents();
        ob_end_clean();
...
function wpestate_testimonial_function($attributes, $content = null)
{
    ...
    if ($type=='type_class_1') {
        ...
        $return_string .= '     </div>';
    } elseif ($type=='type_class_2') {
        $return_string .= '     <div class="testimonial-container '.$type.' ">';
        $return_string .= '     <div class="testimonial-text">'.$testimonial_text.'</div>';
        $return_string .= '     <div class="testimonial-image" style="background-image:url(' .$imagelinks . ')"></div>';
        $return_string .= '     <div class="testimonial-author-line"><span class="testimonial-author">' . $client_name .'</span>, '.$title_client.' </div>';
        $return_string .= '     </div>';
    } elseif ($type=='type_class_3') {
        $return_string .= '     <div class="testimonial-container '.$type.' ">';
        $return_string .= '     <div class="testimonial-image" style="background-image:url(' .$imagelinks . ')"></div>';
