...
class Footer_latest_widget extends WP_Widget
{
    ...
    public function widget($args, $instance)
    {
        ...
        // custom 22/04/2020
        $pg = new PhoneMail(get_the_ID());
        $cash = custome_estate_options($pg->get_option());
        // end custom
        ...
        if ($cash===1) {
            ...
            $price = '<span class=widget_latest_price>'.$price.'</span>';
        } else {
            //if ($cash!=1 && is_user_logged_in()) {
            if (is_user_logged_in()) {
                $price = '<a href="'.$link.'"><button class="wpresidence_button a-btn-asking b-btn-asking sidebar detail">'.__('[:en]See Detail[:km]មើលជាពិស្ដារ[:]').'</button></a>';
            } else {
                $price = '<button class="user_not_loged wpresidence_button a-btn-asking b-btn-asking sidebar login" id="user_menu_u">'.__('[:en]Login for detail[:km]សូមបញ្ជូលគណនី[:]').'</button>';
            }
        }
    ...
    public function action_select_dropn()
    {
    }
}
