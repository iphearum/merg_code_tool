    ...
    function estate_tabbed_interface()
    {
        ...
        print'<div class="property_options_wrapper meta-options">
             ...
             <div class="property_options_wrapper_list">
                <div class="property_tab_item active_tab" data-content="property_agent">'.__('Agent/User', 'wpestate').'</div>
                <div class="property_tab_item" data-content="property_details">'.__('Property Details', 'wpestate').'</div>
        ...
        // print'<div class="property_tab_item" data-content="property_map" id="property_map_trigger">'.__('Map','wpestate').'</div>';
        print'
        ...
        // print'<div class="property_tab_item" data-content="property_paid">'.__('Paid Submission','wpestate').'</div>';
        // print'<div class="property_tab_item" data-content="property_subunits">'.__('Property  Subunits','wpestate').'</div>';
        print'
            </div>
            <div class="property_options_content_wrapper">
                <div class="property_tab_item_content" id="property_details"><h3>'.__('Property Details', 'wpestate').'</h3>';
        estate_box();
        print'</div>
        ...
        print'</div>
                <div class="property_tab_item_content active_tab" id="property_agent"><h3>'.__('Responsible Agent / User', 'wpestate').'</h3>';
...
function custom_details_box()
{
    ...
    if (!empty($custom_fields)) {
        while ($i< count($custom_fields)) {
            ...
            if ($i==(count($custom_fields)-5)) {
                print'<div style="clear:both;background:#FFCC80; padding:0px 5px">
                <h3>Only Franchise for sale</h3>';
            }
            print ' <div class="property_prop_half">   ';
            wpestate_show_custom_field($show, $slug, $name, $label, $type, $order, $dropdown_values, $post_id);
            print '</div>';
...
function details_estate_box($post)
{
    ...
    <td valign="top" align="left">
        ...
        <select id="embed_video_type" name="embed_video_type" style="width: 250px;">
                ' . $option_video . '
        </select>       
        </p>
    </td>';
...
function wpestate_agentestate_box()
{
    ...
    print '
        <div class="property_prop_half">  
        <label for="property_agent">'.__('Agent/Seller: ', 'wpestate').'</label><br />
        <select id="property_agent" style="width: 250px;" name="property_agent">
    ...
    //     <select id="property_user" style="width: 250px;" name="property_user">
    //         <option value=""></option>
    ...
    // print '
    // <div class="property_prop_half">
    // <label for="property_agent_secondary">'.__('Secondary Agents(*multiple selection): ', 'wpestate').'</label>
    // <span>Currently can choose only one</span><br/>
    // <select id="property_agent_secondary" style="width: 250px;height:250px" multiple="multiple" name="property_agent_secondary[]">
    // <option value="">none</option>
    // <option value=""></option>
    // '. $agent_list_sec .'
    // </select>
    ...
    $pg = new PhoneMail($mypost);
    if (is_admin()):
        print '<div class="property_prop_half" id="form_create_agent">
            <label for="c_username">'.__('Form update/create Agent/Seller:', 'wpestate').'</label><br/>
            <div style="border:1px solid #E0E0E0;border-radius: 4px;width: 250px;padding-bottom:7px;padding-right: 3px;padding-left: 3px;">
                <label>Name</label>
                <input type="hidden" id="c_id" name="c_id" value="'.$pg->get_agentId().'">
                <input type="hidden" id="c_mypost" name="c_mypost" value="'.$mypost.'">
                <input style="width: 100%" type="text" id="c_username" name="c_username" placeholder="username" value="'.$pg->get_name().'">
                <label>Email</label>
                <input style="width: 100%" type="text" id="c_email" name="c_email" placeholder="email" value="'.$pg->get_email().'">
                <label>Phone</label>
                <input style="width: 100%" type="text" id="c_phone" name="c_phone" placeholder="phone number" value="'.$pg->all_phone().'">
                <br/><span id="c_alert"></span><br/>
                <bottom type="submit" class="button-primary" id="update_agent">Update Agent</bottom>
                <bottom type="submit" style="margin-left:38px" class="button-create button" id="create_agent">Create Agent</bottom>
            </div></div>'; ?>
        <script>
            $(function(){
                $('bottom[class^="button-"]').click(function(){
                    let c_username = $('div[id="form_create_agent"] input[id="c_username"]').val()
                    let c_email = $('div[id="form_create_agent"] input[id="c_email"]').val()
                    let c_phone = $('div[id="form_create_agent"] input[id="c_phone"]').val()
                    let c_mypost = $('div[id="form_create_agent"] input[id="c_mypost"]').val()
                    let c_type = $(this).attr('id')
                    let c_id = $('div[id="form_create_agent"] input[id="c_id"]').val()
                    var alert = 'this field is require'
                    if(c_username==''||c_phone == ''){
                        ...
                        $('span[id="c_alert"]').html(alert)
                    }else if(c_username!=''||c_phone != ''){
                        $('span[id="c_alert"]').text("loading...")
                        $.ajax({
                            url: 'update_custome_estate_option.php',
                            type: 'post',
                            data: {
...
function estate_box()
{
    // alter_new_table();
    global $post;
    // $property_label = get_property_label();
    wp_nonce_field(plugin_basename(__FILE__), 'estate_property_noncename');
    $mypost = $post->ID;
    ...
    // print'<div class="property_prop_half">
    //     <label for="property_label">'.__('After Price Label(*for example "per month"): ', 'wpestate').'</label><br />
    //     <input type="text" id="property_label" size="40" name="property_label" value="' . esc_html(get_post_meta($mypost, 'property_price', true)) . '">
    // </div>';

    // custome 2020/04/11
    print'<div class="property_prop_half">
    <label for="property_estate_option">'.__('Property Option:', 'wpestate').'</label><br />';
    print'<select id="property_estate_option" style="width: 250px;" name="property_estate_option">';
    $check = false;
    foreach (custome_estate_option() as $vals) {
        print'<optgroup label="'.$vals['option_key'].'">';
        foreach ($vals['option'] as $val) {
            print'<option value="'.$val['estate_value'].'"';
            if (get_post_meta($mypost, 'property_estate_option', true) == $val['estate_value']) {
                $check = true;
                print'selected';
            }
            print'>'.$val['estate_name']."</option>";
        }
        print '</optgroup>';
    }
    if ($check = false && get_post_meta($mypost, 'property_estate_option', true)==null) {
        add_post_meta($mypost, 'property_estate_option', 1, false);
    }
    print'</select><br/>
        <span id="message_property_estate_option" style="position:absolute"></span>
        </div>'; ?>
        <script>
            $(function(){
                $('select[id="property_estate_option"]').change(function(){
                    $('#message_property_estate_option').text("loading...");
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            $('#message_property_estate_option').html( data );
                            // location.reload();
                        }
                    });
                })
        ...
        <?php
        $property_label_before = get_post_meta($mypost, 'property_label_before', true);
    !empty($property_label_before)?$property_label_before=$property_label_before:$property_label_before="USD";
    print'
    <div class="property_prop_half">
        <label for="property_label_before">'.__('Before Price Label(*for example "per month"): ', 'wpestate').'</label><br />
        <input type="text" id="property_label_before" size="40" name="property_label_before" placeholder="US$">
    </div>';
    ...
    // <div class="property_prop_half">
    //     <label for="property_discount">'.__('Discount(*only numbers): ', 'wpestate').'</label><br />
    //     <input type="text" id="property_discount" size="40" name="property_discount" value="' . esc_html(get_post_meta($mypost, 'property_discount', true)) . '">
    // </div>';
}
...
function wpestate_my_columns($columns)
{
    ...
    $columns['estate_option']    = __('Option', 'wpestate');
...
function wpestate_populate_columns($column)
{
    ...
    if ('estate_ID' == $column) {
        echo $the_id;
    } elseif ('estate_agent_email' == $column) {
        $agent_email = get_post_meta($the_id, 'agent_email', true);
        echo $agent_email;
    } elseif ('estate_agent_phone' == $column) {
    ...
    } elseif ('estate_agent_city' == $column) {
        $estate_action = get_the_term_list($the_id, 'property_city_agent', '', ', ', '');
        echo($estate_action);
    } elseif ('estate_agent_action' == $column) {
        $estate_action = get_the_term_list($the_id, 'property_action_category_agent', '', ', ', '');
        echo($estate_action);
    } elseif ('estate_agent_category' == $column) {
        $estate_category =  get_the_term_list($the_id, 'property_category_agent', '', ', ', '');
        echo($estate_category) ;
    } elseif ('estate_option' == $column) {
        print '<select class="property_estate_option" id="'.$the_id.'" style="width:100%">';
        foreach (custome_estate_option() as $vals) {
            print'<optgroup label="'.$vals['option_key'].'">';
            foreach ($vals['option'] as $val) {
                print'<option value="'.$val['estate_value'].'"';
                if (get_post_meta($the_id, 'property_estate_option', true) == $val['estate_value']) {
                    print'selected';
                }
                print'>'.$val['estate_name']."</option>";
            }
        ...
        print'</select>';
        print'<span id="message_property_estate_option_'.$the_id.'"></span>';
    } elseif ('estate_status' == $column) {
    ...
    } elseif ('estate_autor' == $column) {
        $temp_id= $post->ID;
        $user_id=wpsestate_get_author($the_id);
        $estate_autor = get_the_author_meta('display_name', $user_id);
        $pg = new PhoneMail($temp_id);
        echo '<span>Creator</span> :<br/><a href="'.get_edit_user_link($user_id).'" >'.$estate_autor.'</a><hr>';
        echo '<span>Seller</span> :<br/>'.$pg->get_name();
      
        $post->ID=$temp_id=$the_id;
    } elseif ('estate_thumb' == $column  || 'estate_agent_thumb' == $column) {
    ...
    } elseif ('estate_city' == $column) {
        $estate_city = get_the_term_list($the_id, 'property_city', '', ', ', '');
        echo($estate_city);
    } elseif ('estate_action' == $column) {
    ...
    echo '<script>
            $(function(){
                $("select.property_estate_option").change(function(){
                    var selected = $(":selected", this).val();
                    var id = $(this).attr("id");
                    $("#message_property_estate_option_"+id).text("loading...");
                    $.ajax({
                        url: "update_custome_estate_option.php",
                        type: "post",
                        data: {
                        ...
                        success: function(data) {
                            $("#message_property_estate_option_"+id).html("Success!");
                        }
                    });
                })
            });
...
add_action('property_category_edit_form_fields', 'wpestate_property_city_callback_function', 10, 2);
add_action('property_category_add_form_fields', 'wpestate_property_city_callback_add_function', 10, 2);
...
add_action('property_action_category_edit_form_fields', 'wpestate_property_city_callback_function', 10, 2);
