...
jQuery(document).ready(function(c) {
    ...
    if (typeof(plupload) !== "undefined") {
        ...
        d.bind("UploadProgress", function(e, f) {
            c("#" + f.id + " b").html(f.percent + "%")
        });
        d.bind("Error", function(e, f) {
            c("#aaiu-upload-imagelist").append("<div>Error: " + f.code + ", Message: " + f.message + (f.file ? ", File: " + f.file.name : "") + "</div>");
            e.refresh()
        });
        d.bind("FileUploaded", function(f, i, h) {
            ...
            c("#" + i.id).remove();
            if (e.success) {
                c("#profile-image").attr("src", e.html);
                c("#profile-image").attr("data-profileurl", e.html);
                c("#profile-image").attr("data-smallprofileurl", e.attach);
                var g = c("#attachid").val();
                g = g + "," + e.attach;
                c("#attachid").val(g);
                if (e.html !== "") {
                    if (ajax_vars.is_floor === "1") {
                        c("#no_plan_mess").remove();
                        c("#imagelist").append('<div class="uploaded_images floor_container" data-imageid="' + e.attach + '"><input type="hidden" name="plan_image_attach[]" value="' + e.attach + '"><input type="hidden" name="plan_image[]" value="' + e.html + '"><img src="' + e.html + '" alt="thumb" /><i class="fa deleter fa-trash-o"></i>' + to_insert_floor + "</div>")
                    } else {
                        c("#imagelist").append('<div class="uploaded_images" data-imageid="' + e.attach + '"><img src="' + e.html + '" alt="thumb" /><i class="fa deleter fa-trash-o"></i> </div>')
                    }
                } else {
                    c("#imagelist").append('<div class="uploaded_images" data-imageid="' + e.attach + '"><img src="' + ajax_vars.path + '/img/pdf.png" alt="thumb" /><i class="fa deleter fa-trash-o"></i> </div>')
                }
                if (jQuery().sortable) {
                    c("#imagelist").sortable({
                        revert: true,
                        update: function(l, m) {
                            var k, j;
                            k = "";
                            c("#imagelist .uploaded_images").each(function() {
                ...
                if (e.image) {
                    c("#imagelist").before('<div id="image_warn" style="width:100%;float:left;">' + ajax_vars.warning + "</div>")
                }
            }
        });
