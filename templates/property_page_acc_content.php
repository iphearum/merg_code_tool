...
if ($content !='' && !is_int(strpos($content,"Sorry, this entry is only available in"))) {
    print '<div class="wpestate_property_description">'.$content.'</div>';
}
else {
    $temp = $wpdb->get_results("SELECT * FROM $wpdb->posts WHERE id = ".$post->ID);
    $temp2 = $temp[0]->post_content;
    $temp2 = str_replace(']]>', ']]&gt;', $temp2);
    $replay = array("[:km]", "[:en]", "[vc_column]", "[/vc_column]", "[/vc_column_text]", "[vc_column_text]","[vc_row]", "[/vc_row]", "[:]");
    $temp2 = str_replace($replay, "", $temp2);
    $content = apply_filters('the_content', $temp2);
    print '
...
<div class="panel-group property-panel" id="accordion_prop_features">  
    <div class="panel panel-default">
        ...
        <div id="collapseThree" class="panel-collapse collapse in">
          <div class="panel-body">
          <?php 
          print estate_listing_features($post->ID); ?>
          </div>
        </div>
    </div>
</div>  
...
//                 <h4 class="panel-title">'._e('Page Views Statistics', 'wpestate').'</h4>
//                </a>
//            </div>
